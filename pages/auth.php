    ...
    <form action="<?php $_SERVER['REQUEST_URI'] ?>" method="POST">
      <div class="overflow-hidden sm:rounded-2xl border border-slate-200">
        ...
        <div class="px-4 py-3 bg-gray-800 text-right sm:px-6">
          <button type="submit" name="send" class="inline-flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">Авторизоваться</button>
        </div>
      </div>
    </form>
  </div>
...
<?php else: ?>
  <div class="max-w-6xl m-auto mt-10">
    <div class="border-l-2 border-l-blue-400 pl-2 mb-10 text-2xl font-bold">Вы авторизованы: <?php echo $_SESSION['username']; ?> </div>
    <div class="px-4 py-3 bg-gray-800 text-left sm:px-6 rounded-xl">
